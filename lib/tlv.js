...
const fields = ['tag', 'name', 'format', 'length', 'description', 'comments', 'defaultValue', 'detailed', 'tagOwner', 'isPublic', 'createdAt', 'modifiedAt', 'timesRequested', 'exist'];
...
exports.add = (tlv) => {
    ...
    for (const key in tlv) {
        if (editableFields.includes(key) && tlv[key] != null && tlv[key].trim() != '')
            update += `${key} = '${tlv[key]}', `;
    }
